...
namespace Nest.BaseCore.Payment.PaymentSDK.Wechat
{
    public class WxPayApi
    {
        ...
        */
        public static WxPayData Micropay(WxPayData inputObj, string clientIp = "", int timeOut = 10)
        {
            ...
            inputObj.SetValue("spbill_create_ip", string.IsNullOrEmpty(clientIp) ? WechatConfig.IP : clientIp);//终端ip
            inputObj.SetValue("appid", WechatConfig.AppId);//公众账号ID
            inputObj.SetValue("mch_id", WechatConfig.MchId);//商户号
        ...
        */
        public static WxPayData OrderQuery(WxPayData inputObj, string key = WechatConfig.Key, int timeOut = 6)
        {
            ...
            //LogHelper.WriteLog("OrderQuery request : " + xml);
            string response = HttpService.Post(xml, url, false, timeOut, "");//调用HTTP通信接口提交数据
            //LogHelper.WriteLog("OrderQuery response : " + response);
            ...
            ReportCostTime(url, timeCost, result);//测速上报
            //LogHelper.WriteLog("测速上报结果:"+result.ToXml());
            return result;
        }
        ...
        */
        public static WxPayData Refund(WxPayData inputObj, int timeOut = 6, bool isApp = false, string key = WechatConfig.Key)
        {
            ...
            if (!inputObj.IsSet("appid"))
            {
                if (isApp)
                {
                    inputObj.SetValue("appid", WechatConfig.AppIdByApp);//公众账号ID
                }
                else
                {
                    inputObj.SetValue("appid", WechatConfig.AppId);//公众账号ID
            ...
            if (!inputObj.IsSet("mch_id"))
            {
                if (isApp)
                {
                    inputObj.SetValue("mch_id", WechatConfig.MchIdByApp);//商户号
                }
                else
                {
                    inputObj.SetValue("mch_id", WechatConfig.MchId);//商户号
            ...
            string response = HttpService.Post(xml, url, true, timeOut, inputObj.GetValue("appid").ToString());
	    ...
	    * @return 成功时返回，其他抛异常
	    */
        public static WxPayData RefundQuery(WxPayData inputObj, int timeOut = 6000)
        {
        ...
        * @return 成功时返回，其他抛异常
        */
        public static WxPayData UnifiedOrder(WxPayData inputObj, string notifyUrl, int timeOut = 6, string key = WechatConfig.Key)
        {
            ...
            {
                inputObj.SetValue("spbill_create_ip", WechatConfig.IP);//终端ip	  
            }
            inputObj.SetValue("nonce_str", GenerateNonceStr());//随机字符串
            ...
            //LogHelper.LogInfo("WxPayApi", "UnfiedOrder request :" + xml);
            string response = HttpService.Post(xml, url, false, timeOut, "");
       ...
       * @return 成功时返回，其他抛异常
       */
        public static WxPayData WeiXinMPay(WxPayData inputObj, int timeOut = 6)
        {
            string url = "https://api.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers";

            inputObj.SetValue("mch_appid", WechatConfig.AppId);//公众账号ID
            inputObj.SetValue("mchid", WechatConfig.MchId);//商户号
            inputObj.SetValue("spbill_create_ip", WechatConfig.IP);//终端ip	  	    
            inputObj.SetValue("nonce_str", GenerateNonceStr());//随机字符串
	    ...
	    */
        private static void ReportCostTime(string interface_url, int timeCost, WxPayData inputObj)
        {
            ...
            //如果仅失败上报
            if (WechatConfig.REPORT_LEVENL == 1 && inputObj.IsSet("return_code") && inputObj.GetValue("return_code").ToString() == "SUCCESS" &&
	    ...
	    */
        public static WxPayData Report(WxPayData inputObj, int timeOut = 1)
        {
            ...
            inputObj.SetValue("appid", WechatConfig.AppId);//公众账号ID
            inputObj.SetValue("mch_id", WechatConfig.MchId);//商户号
            inputObj.SetValue("user_ip", WechatConfig.IP);//终端ip
